-- Migration 066 ("fix_market_purchase_fee_zero") replaced execute_market_purchase
-- to split the business_accounts inserts (fixing a real bug), but in doing so it
-- was rewritten from a pre-058 copy of the function and silently dropped the
-- personal-seller branch that migration 058 ("market_personal_listing_sources")
-- had added: crediting the seller's personal checking account via `transactions`
-- when the listing's source_type is 'personal', instead of business_accounts.
-- Every subsequent "create or replace" of this function (068's pass-through, 079's
-- cost-basis change) carried the regression forward, so as of 079 buying ANY
-- personal-inventory-sourced listing throws "null value in column business_id
-- violates not-null constraint" (business_accounts.business_id is NOT NULL, and
-- v_listing.source_business_id is null for personal listings) and the whole
-- purchase rolls back. Found incidentally while building NPC open-market
-- purchases (see 082/083), which sell against both source types and would have
-- silently inherited the same bug for personal-sourced listings.
--
-- Also restores writing `seller_source_type` on the market_transactions insert —
-- 066 dropped that column from the insert list too, so every transaction since
-- has silently recorded 'business' (the column default) regardless of the
-- listing's actual source_type.
