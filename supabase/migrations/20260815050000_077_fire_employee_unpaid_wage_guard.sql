-- fire_employee_atomic deleted the employee row unconditionally, including when
-- unpaid_wage_due was still outstanding, silently erasing the debt with no
-- forgiveness/write-off ledger entry. Require wages to be settled before an
...
begin
  ...
  if coalesce(v_employee.unpaid_wage_due, 0) > 0 then
    raise exception 'Cannot fire an employee with unpaid wages. Settle wages first.';
  end if;
