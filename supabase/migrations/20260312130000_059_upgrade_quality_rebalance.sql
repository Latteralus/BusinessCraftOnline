-- Phase 23 patch: remove input reduction, rebalance manufacturing upgrades,
-- and shift item quality to a 0..100 scale with a 0 baseline.
...
set
  base_effect = 5,
  updated_at = now()
where upgrade_key = 'equipment_quality';

delete from public.upgrade_definitions
where upgrade_key = 'input_reduction';

update public.business_upgrade_projects
set
  project_status = 'cancelled',
  updated_at = now()
where upgrade_key = 'input_reduction'
  and project_status in ('queued', 'installing');

alter table public.personal_inventory
  alter column quality set default 0;

alter table public.personal_inventory
  drop constraint if exists personal_inventory_quality_check;

alter table public.personal_inventory
  add constraint personal_inventory_quality_check
...
alter table public.business_inventory
  drop constraint if exists business_inventory_quality_check;

alter table public.business_inventory
  add constraint business_inventory_quality_check
  check (quality between 0 and 100);

alter table public.market_listings
  drop constraint if exists market_listings_quality_check;

alter table public.market_listings
  add constraint market_listings_quality_check
  check (quality between 0 and 100);

alter table public.market_transactions
  drop constraint if exists market_transactions_quality_check;

alter table public.market_transactions
  add constraint market_transactions_quality_check
  check (quality between 0 and 100);

alter table public.store_shelf_items
  drop constraint if exists store_shelf_items_quality_check;

alter table public.store_shelf_items
  add constraint store_shelf_items_quality_check
...
set quality = greatest(0, quality - 40)
where quality <> greatest(0, quality - 40);
