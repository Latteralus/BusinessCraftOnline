-- Allow cross-purchases between different sources owned by the same player.
-- Only block buying a listing from the exact same business that created it.
...
begin
  ...
  if v_listing.source_business_id = v_buyer_business.id then
