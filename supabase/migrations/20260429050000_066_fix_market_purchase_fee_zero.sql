-- Phase 66: Fix execute_market_purchase crashing when market fee rounds to $0.00.
--
-- The original function inserted all three business_accounts rows (seller credit,
-- seller fee debit, buyer debit) in a single VALUES statement.  When gross_total
-- is small enough that round(gross * 0.03, 2) = 0.00, the insert fails with
-- "new row for relation business_accounts violates check constraint
-- business_accounts_amount_check" (amount > 0) and rolls back the entire purchase.
-- Items with gross_total < ~$0.17 (1 unit × price < $0.17) were therefore
-- unpurchasable by businesses.
--
-- Fix: split into three separate inserts; guard the fee row behind IF v_fee > 0.
--
-- Market transaction fee rate is 3 % (MARKET_TRANSACTION_FEE = 0.03 in shared/economy.ts).
-- Kept as a local constant here; update both places if the rate ever changes.
