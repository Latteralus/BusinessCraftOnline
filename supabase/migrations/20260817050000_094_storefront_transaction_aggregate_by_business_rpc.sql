-- Resolves audit finding M3 (Documents/SBAudit.md): getStorefrontPerformanceSummary
-- (src/domains/market/service.ts) called get_storefront_transaction_aggregate once
-- per owned business to build the per-business breakdown. Add a grouped variant
-- that returns one row per business in a single call.
...
revoke all on function public.get_storefront_transaction_aggregates_by_business(uuid, timestamptz, timestamptz) from public;
grant execute on function public.get_storefront_transaction_aggregates_by_business(uuid, timestamptz, timestamptz) to authenticated;
