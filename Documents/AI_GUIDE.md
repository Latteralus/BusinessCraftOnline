...
- City/world labor pressure and wage-expectation formulas live in `shared/cities/labor.ts` (`getCityLaborPressure`/`getEffectiveCityLaborIndex`/`getExpectedWage`). Dynamic state feeding them (`city_economic_state`, `world_economic_state`) is maintained once/24h by `run_government_daily_update()` via `tick-government-daily`, never written from route handlers. Nothing calls `getExpectedWage` yet -- see `Documents/Plans/CityPlan.md` Phase 2 and `Documents/changelog.md`.
- Municipal stockpile lazy-consumption math lives in `shared/cities/stockpiles.ts`, mirrored in SQL by `_materialize_city_stockpile_ids()`. `city_stockpiles` (owned by the new `government` domain, not `cities-travel`) is a checkpoint (`stored_quantity` as of `last_materialized_at`), not current truth -- always read through `government`'s `getProjectedCityStockpiles()` rather than trusting `stored_quantity` directly. See `Documents/Plans/CityPlan.md` Phase 3.
